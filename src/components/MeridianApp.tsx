...
const MapView = dynamic(() => import('@/components/MapView').then((module) => module.MapView), {
  ssr: false,
  loading: () => (
    <div className="meridian-panel h-[100dvh] w-full rounded-none bg-white/40 md:rounded-[2rem]" />
  )
});
  ...
  return (
    <div className="pointer-events-none flex items-start justify-between gap-4">
      <div className="meridian-panel pointer-events-auto max-w-md rounded-[1.75rem] px-5 py-4">
        ...
        <div className="mt-1 text-sm text-zinc-500">一个双人使用的私密旅行记录网站</div>
      </div>
      ...
      {open ? (
        <>
          <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-black text-xl text-white shadow-lg">
              📍
            </div>
          </div>
          <motion.div
        ...
        <div className="mt-5 min-h-0 flex-1 overflow-y-auto pr-1">
          <div className="space-y-4">
            <label className="block">
              <div className="mb-2 text-sm text-zinc-600">标题</div>
              <input className="meridian-input" value={title} onChange={(event) => setTitle(event.target.value)} />
            </label>

            <label className="block">
              <div className="mb-2 text-sm text-zinc-600">访问日期</div>
              <input
                type="date"
            ...
            <div>
              <div className="mb-2 text-sm text-zinc-600">正文</div>
            ...
            <div>
              ...
              <label className="block cursor-pointer rounded-[1.25rem] border border-dashed border-black/10 bg-white/70 px-4 py-6 text-center text-sm text-zinc-500">
                粘贴、拖拽或点击上传图片
                <input
                  type="file"
                  accept="image/*"
              ...
              {thumbnails.length > 0 ? (
                <div className="mt-3 grid grid-cols-3 gap-3">
                  {thumbnails.map((thumbnail, index) => (
                    <div key={thumbnail} className="relative overflow-hidden rounded-2xl border border-black/5">
...
function AuthorCombobox({ value, options, onChange }: AuthorComboboxProps) {
  return (
    <label className="block">
      <div className="mb-2 text-sm text-zinc-600">署名</div>
      <input
